Public Class frmPO
    ...
    Private Sub ShowDataPR()
        Using cn As New SqlClient.SqlConnection(strCOnnLocal)
            Using cmd As New SqlClient.SqlCommand
                ...
                Try
                    cmd.CommandText =
                    "select a.TransID,a.PONo,a.PRNo,EndUserName,a.KodeBarang,NamaBarang,a.UOM,Qty,a.LastPrice,EstTotPrice,a.PurchasedByGA,'Add'BtnAdd " &
                    "from tbATKPODetails a inner join tbATKPOMain b on a.PONo=b.PONo inner join tbATKmstbrg c on a.KodeBarang=c.KodeBarang  " &
                    "where b.Status =0 order by a.PONo,a.TransID "
            ...
            If MsgBox("Anda yakin akan menghapus Transaksi ini", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                ...
                Using cn As New SqlClient.SqlConnection(strCOnnLocal)
                    Using cmd As New SqlClient.SqlCommand
                        cmd.Connection = cn
                        cn.Open()
                        cmd.CommandText = "INSERT INTO tbATKPODetails (PONo,PRNo,EndUserName,KodeBarang,UOM,Qty,KodeVendor,Keterangan,LastPrice,EstTotPrice,PurchasedByGA) VALUES (@PONo,@PRNo,@EndUserName,@KodeBarang,@UOM,@Qty,@KodeVendor,@Keterangan,@LastPrice,@EstTotPrice,@PurchasedByGA)"

                        cmd.Parameters.Clear()
                        cmd.Parameters.Add(New SqlClient.SqlParameter("PONo", txtPONo.Text.Trim))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("PRNo", Me.Dgv.Item("PRNo", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("EndUserName", Me.Dgv.Item("Customer", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("KodeBarang", Me.Dgv.Item("KodeBarang", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("UOM", Me.Dgv.Item("UOM_PR", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("Qty", Me.Dgv.Item("Qty", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("KodeVendor", Me.cbCompany.SelectedValue))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("Keterangan", Me.Dgv.Item("NamaBarang", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("LastPrice", Me.Dgv.Item("LastPrice", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("EstTotPrice", Me.Dgv.Item("EstTotPrice", Me.Dgv.CurrentRow.Index).Value))
                        cmd.Parameters.Add(New SqlClient.SqlParameter("PurchasedByGA", Me.Dgv.Item("PurchasedByGA", Me.Dgv.CurrentRow.Index).Value))
                ...
                Using cn As New SqlClient.SqlConnection(strCOnnLocal)
                    Using cmd As New SqlClient.SqlCommand()
                        cmd.Connection = cn
                        cn.Open()
                        cmd.CommandText = "delete tbATKPODetails where TransID=@TransID"
                        cmd.Parameters.Clear()
                        cmd.Parameters.Add(New SqlClient.SqlParameter("TransID", Me.Dgv.Item("idPR", Me.Dgv.CurrentRow.Index).Value))
    ...
    Private Sub ComboBox1_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles cbCompany.KeyPress
        If Asc(e.KeyChar) = 13 Then
            ...
            Me.txtTlp.Text = IIf(IsDBNull(ds.Tables(0).Rows(0)("Tlp")), "0", ds.Tables(0).Rows(0)("Tlp"))
            Me.TxtAttnTo.Text = IIf(IsDBNull(ds.Tables(0).Rows(0)("Attn")), "", ds.Tables(0).Rows(0)("Attn"))
        End If
    End Sub
        ...
        Using cn As New SqlClient.SqlConnection(strCOnnLocal)
            Using cmd As New SqlClient.SqlCommand
                Try
                    ...
                    cmd.Connection = cn
                    cmd.CommandText =
                    "select * from tbATKmstCompany where CompanyName ='" & strCompany & "'"
                    Dim da As New SqlClient.SqlDataAdapter(cmd)
                    da.Fill(ds, "hasil")
                Catch ex As Exception
                    ds = New DataSet
    ...
    Private Sub INSERTDATAMASTER()
        ...
        Using cn As New SqlClient.SqlConnection(strCOnnLocal)
            Using cmd As New SqlClient.SqlCommand
                ...
                cmd.CommandText =
                        "Update tbATKPOMain set KodeVendor=@KodeVendor,VendorName=@VendorName,Notes=@Notes,editwho=@editwho where PONo=@PONo "
                cmd.Parameters.Clear()
                ...
                ElseIf strUserName = "wkurniati" Then
                    cmd.Parameters.Add(New SqlClient.SqlParameter("EditWho", "Wiwik Kurniati"))
                Else
    ...
    Private Sub ShowDataMaster(ByVal TransID As Integer)
        Using cn As New SqlClient.SqlConnection(strCOnnLocal)
            Using cmd As New SqlClient.SqlCommand
                ...
                Try
                    cmd.CommandText =
                    "Select isnull(a.TransID,'0')TransID,a.PONo,WHAsal,a.KodeVendor,CompanyName,isnull(Address1,'')Address1,isnull(Address2,'')Address2,isnull(Address3,'')Address3,case isnull(Status,0) when 0 then 'Created' when 2 then 'Waiting NSO Approval' " &
                    "when 3 then 'Approved by NSO' when 4 then 'Waiting GM Approval' when 5 then 'Approved by GM' when 6 then 'Waiting CFO Approval' when 8 then 'Approved by CFO' when 9 then 'Cancelled' else '' end Status,InputBy, " &
                    "isnull(Notes,'')Notes,isnull(StartDate,getdate())StartDate,isnull(EndDate,getdate())EndDate,DateRequired,isnull(TypePO,'')TypePO  from tbATkPOMain a " &
